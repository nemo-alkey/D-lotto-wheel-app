...
def _reset_quantum_kernel_cache() -> None:
    """
...
def _force_width(mat: npt.ArrayLike, width: int, name: str) -> npt.NDArray[np.float64]:
    ...
    """
    mat = _ensure_2d(mat, name)  # Ensures I can safely index shape
    n, d = mat.shape  # n rows, d columns

    if d == width:  # If already correct width, do nothing
        return mat.astype(float)  # Ensures dtype is float

    out = np.zeros((n, width), dtype=float)  # Allocates padded/truncated output
    m = min(d, width)  # Overlaps region length
    out[:, :m] = mat[:, :m]  # Copys overlap columns

    logging.warning(
        f"{name} width {d} != {width}; padded/trimmed to {width}."
    )  # Notifys shape correction
...
def deep_learning_prediction(pipeline: DataPipeline) -> None:
    ...
    )  # Bayesian fused and normalised probabilities
    ...
    if any(v is None for v in required):  # Abort if any upstream feature missing
        logging.error(
            "Deep learning aborted: missing required pipeline features."
        )  # Emit diagnostics
        pipeline.add_data(
    ...
    y_all = np.asarray(labels, dtype=float)  # Stack labels into shape (n_draws, 50)
    n_draws = y_all.shape[0]  # Number of historical examples available
    ...
    for t in range(n_draws):  # Walk forward through history
        ...
        freqs[t] = (
            counts / s if s > 0 else np.ones(NUM_TOTAL) / NUM_TOTAL
        )  # Convert counts to frequencies or uniform

        for n in historical_data[t].get(
            "numbers", []
        ):  # Adds numbers from the current draw into counts
    ...
    x_train, x_val = (
        x_all[:-n_val],
        x_all[-n_val:],
    )  # Train on early history, validate on most recent history
    y_train, y_val = y_all[:-n_val], y_all[-n_val:]  # Same split for labels
    ...
    cw = np.clip(cw, MIN_CLASS_WEIGHT, MAX_CLASS_WEIGHT).astype(
        np.float32
    )  # Clamp to keep gradients sane

    class_weights = cw  # Store NumPy version globally
    class_weights_tf = tf.constant(
        class_weights, dtype=tf.float32
    )  # Store TF constant for loss function
    ...
    try:
        train_quantum_encoder(
            x_train, y_train
        )  # Fit/tune the quantum encoder using training data only
        logging.info("Quantum encoder training complete.")  # Confirm completion
    except Exception as e:
        logging.warning(
            f"Quantum encoder training failed: {e}"
        )  # Continue even if quantum training fails
    ...
    try:
        ...
        q_val = np.zeros(
            (x_val.shape[0], QUANTUM_FEATURE_LEN)
        )  # Fallback to zeros with correct shape
    ...
    try:
        ...
        k_train = _force_width(
            k_train_raw, KERNEL_PROTOTYPES, "K_train"
        )  # Enforce fixed width on training kernel feats
        k_val = _force_width(
    ...
    xf_train = np.column_stack((x_train, q_train, k_train)).astype(
        float
    )  # Fuse classical + quantum + kernel (train)
    xf_val = np.column_stack((x_val, q_val, k_val)).astype(
        float
    ...
    model = keras.Sequential(
        [  # Simple feedforward network for tabular fused features
            ...
            keras.layers.Dense(
                NUM_TOTAL, activation="sigmoid"
            ),  # Output layer: independent probs per class (multi-label)
        ]
    )

    model.compile(
        optimizer=keras.optimizers.Adam(
            learning_rate=8e-4, clipnorm=1.0
        ),  # Adam optimiser with moderately high LR
    ...
    f_now = (
        (  # Construct current frequency vector
            counts / s
            if s > 0
            else np.ones(NUM_TOTAL) / NUM_TOTAL  # Normalised counts or uniform fallback
        )
        .reshape(1, -1)
        .astype(float)
    )  # Convert to row vector for feature construction

    x_now = np.column_stack(
        (  # Build current classical feature row in the same structure as training x_all
    ...
    xf_now = np.column_stack((x_now, q_now, k_now)).astype(
        float
    )  # Fuse classical + quantum + kernel for inference

    if (
        xf_now.shape[1] != input_dim
    ):  # Hard check: inference feature width must match model input width
        logging.error(
    ...
    try:
        dl_pred = (
            model.predict(xf_now, verbose=0).reshape(-1).astype(float)
        )  # Run prediction and flatten to (50,)
    except Exception as e:
    ...
    pipeline.add_data(
        ...
        _prob_norm_vec(
            np.clip(dl_pred, 0.0, 1.0), "deep_learning_predictions"
        ),  # Ensure final probabilities are within [0, 1]
    )
