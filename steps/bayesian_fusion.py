...
def _estimate_mechanics_dirichlet_from_history(
    ...
    return (
        ...
        float(chi2),
        int(total_counts),
    )  # Return posterior, chi2 value, and sample size
...
def bayesian_fusion_with_mechanics(
    ...
    mechanics_vec, chi2_stat, total_obs = _estimate_mechanics_dirichlet_from_history(
    ...
    if use_mechanics_if_significant:  # Only apply mechanics if statistically meaningful
        if total_obs == 0 or chi2_stat < chi2_threshold:
            ...
            if verbose:
                logging.info(
                    f"Mechanics not significant (chi2={chi2_stat:.2f}, n={total_obs}). Using uniform mechanics."
                )
        else:
            if verbose:
                logging.info(
                    f"Mechanics significant (chi2={chi2_stat:.2f}, n={total_obs}). Using estimated mechanics."
                )
    ...
    posterior_norm = posterior / max(
        posterior.max(), 1e-12
    )  # Scale max value to 1 for feature input
    ...
    pipeline.add_data(
        "mechanics_used_is_uniform", bool(mechanics_is_uniform)
    )  # Whether mechanics was suppressed
