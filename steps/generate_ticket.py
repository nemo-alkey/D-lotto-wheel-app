...
from block_analysis import validate_positional_ranges
...
def safe_norm(x: npt.ArrayLike) -> npt.NDArray[np.float64]:
    """
...
def generate_ticket(
    ...
    block_ranges: dict[str, Any] | None = None,
    dynamic_sum_range_tuple: tuple[float, float] | None = None,
) -> list[dict[str, Any]]:
    """
    Generate NUM_LINES lottery lines using:
      ...
      - positional block analysis filtering (when block_ranges is provided)
      - dynamic sum-range validation (when dynamic_sum_range_tuple is provided)

    Parameters
    ----------
    pipeline : Pipeline
        The pipeline object providing deep_learning_predictions.
    block_ranges : dict or None
        Output from block_analysis.compute_block_ranges().  When provided,
        candidate lines are filtered to match at least 4 positional buckets.
    dynamic_sum_range_tuple : tuple[float, float] or None
        (min_sum, max_sum) from sum_analysis.dynamic_sum_range().  When
        provided, candidate lines must have a sum within this range.
    ...
    predictions = pipeline.get_data(
        "deep_learning_predictions"
    )  # Retrieve model output from pipeline
    ...
    if predictions is None or len(predictions) != expected_len:  # Validate prediction vector
        ...
        base_main_prob = safe_norm(
            predictions[:NUM_MAIN_NUMBERS]
        )  # Extract + normalise main-number probabilities
        base_pb_prob = safe_norm(
            predictions[NUM_MAIN_NUMBERS:]
        )  # Extract + normalise Powerball probabilities
    ...
    for _ in range(NUM_LINES):  # Loop once per ticket line
        ...
        for _ in range(MAX_RESAMPLE_TRIES):  # Rejection-sampling loop
            ...
            )  # Combine prediction + usage penalty
            effective_pb_prob = safe_norm(
                base_pb_prob * pb_penalty
            )  # Combine prediction + usage penalty
            ...
            # Positional block constraint (from Albert's Block Analysis)
            if block_ranges is not None and not validate_positional_ranges(
                cand_main, block_ranges, min_positions=4
            ):
                continue

            # Dynamic sum-range constraint
            if dynamic_sum_range_tuple is not None:
                min_sum, max_sum = dynamic_sum_range_tuple
                cand_sum = sum(cand_main)
                if cand_sum < min_sum or cand_sum > max_sum:
                    continue
