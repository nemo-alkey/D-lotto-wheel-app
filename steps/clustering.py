...
def kmeans_clustering_and_correlation(
    ...
    fusion = pipeline.get_data(
        "bayesian_fusion"
    )  # Retrieve fused probability vector from pipeline
    ...
    data_main = scaler_main.fit_transform(
        fusion_main.reshape(-1, 1)
    )  # Reshape for sklearn (40,1)
    ...
    try:
        ...
        kmeans_main = KMeans(
            n_clusters=int(n_clusters_main), random_state=42, n_init=10
        )
        labels_main = kmeans_main.fit_predict(data_main).astype(
            int
        )  # Cluster ID for each number (40,)
        centers_main = np.asarray(
            kmeans_main.cluster_centers_, dtype=float
        )  # Cluster center values (K_main,1)

        # Map each number to its cluster center value
        centroids_main = centers_main[labels_main].reshape(
            -1
        )  # Per-number centroid strength
    ...
    try:
        kmeans_power = KMeans(
            n_clusters=int(n_clusters_powerball), random_state=42, n_init=10
        )
        labels_power = kmeans_power.fit_predict(data_power).astype(
            int
        )  # Cluster ID per PB number
        centers_power = np.asarray(
            kmeans_power.cluster_centers_, dtype=float
        )  # Cluster center strengths

        centroids_power = centers_power[labels_power].reshape(
            -1
        )  # Per-number PB centroid values
