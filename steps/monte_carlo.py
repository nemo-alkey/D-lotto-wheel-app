...
def adjust_probabilities(
    fusion_probs: npt.ArrayLike, centroids: npt.ArrayLike, clusters: npt.ArrayLike
) -> npt.NDArray[np.float64]:
    """
    Adjust probabilities using Bayesian fusion + clustering.
    """
    fusion_probs = np.asarray(fusion_probs, dtype=np.float64)  # Ensure float array
...
def calculate_distribution(
    ...
    """
    picks_array = np.asarray(picks_array, dtype=np.int64)  # Ensure integer picks
...
def monte_carlo_simulation(pipeline: DataPipeline) -> None:
    """
